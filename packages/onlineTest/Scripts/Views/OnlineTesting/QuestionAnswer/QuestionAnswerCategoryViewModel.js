...
var viewModel = kendo.observable({
    ...
    TestToolByQuestionCategory: function (e) {
        TestToolByQuestionCategory(e);
    },
    QnACode: "",
    QuestionCategory: "",
    TestTypeName: "",
    ...
    WorkingMemory: {
        ...
        WorkingMemoryDurationTransition: "0",
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.Acknowledge < 1) {
                            ...
                            if(valueMedia.toString() == "0"){viewModel.set("QuestionImage", Url);}
...
Simpan = function () {
    ...
    const IsDecimal = (num) => num.toString().includes(".");
    if (Name == '' || Name == undefined || Duration == undefined || Story == '' || Story == undefined) {
        swal("Incompleted Data", "Please Fill Mandatory Field", "warning", { closeOnClickOutside: false });
    }
    else {
        if (IsDecimal(Duration)) {
...
save = function (e){ 
    ...
    if (QC == 'NORMAL') {
        ...
        if (typeAnswer == "AT19000001") {
            ...
            //var checkinputAllScoreZero = ifAnyNonZero(AnswerNormal, lengthNormalText);
            var valid;
            //if(!checkinputAllScoreZero){
                valid = checkValidationForm(AnswerNormal, lengthNormalText);
            //}else{
            //    valid = false;
            //}
            ...
            //        } 
            //    } else {
            //        if (AnswerNormal[kll].Text != "" && (AnswerNormal[kll].AnswerScore !== "" && AnswerNormal[kll].AnswerScore >= 0)) {
            //            valid = true;
            //        }
            //        else {
            //            valid = false;
                        ...
                        $('.swal-button--defeat').on('click', function () {
                            ...
                            $.ajax({
                                ...
                                data: {
                                    ...
                                    QuestionImage: viewModel.QuestionImage,
            ...
            var imageQuestion = viewModel.QuestionImage;
                var data = [];
                ...
                var valid = true;
                    for (s = 0; s < lengthImage; s++) {
                        if(s < 2){
                            if (viewModel.Normal.NormalImage[s].NormalScoreImage === "" || viewModel.Normal.NormalImage[s].NormalAnswerImage === "") {
                                valid = false;
                                break;
                            } else {
                                valid = true;
                            ...
                            if (viewModel.Normal.NormalImage[s].NormalAnswerImage !== "" && viewModel.Normal.NormalImage[s].NormalScoreImage !== "") {
                                valid = true;
                                AnswerNormal[s] = {
                                    No: nomorAnswer,
                                    Text: '',
        ...
        //var checkinputAllScoreZero = ifAnyNonZero(WorkingMemoriList, lengthWorking);
        var Valid;
        //if(!checkinputAllScoreZero){
            Valid = checkValidationForm(WorkingMemoriList, lengthWorking);
        //}else{
        //    Valid = false;
        //}
        debugger
        ...
        //    if (kll < 2) {
        //        if (WorkingMemoriList[kll].Text == "" || WorkingMemoriList[kll].AnswerScore == "" || WorkingMemoriList[kll].AnswerScore <= 0) {
        //            Valid = false;
        //            break;
        //        } 
        //    } else {
            ...
            if (Valid == false) {
                ...
                if (SubTestCode == '' || SubTestCode == undefined ||
                    ...
                    if (WorkingMemoryDuration <= 0) {
                        swal("Invalid!", "Duration Per Memory value must more than 0", "warning", { closeOnClickOutside: false });
                        ...
                        const IsDecimal = (num) => num.toString().includes(".");
                        if (IsDecimal(WorkingMemoryDurationTransition) || IsDecimal(WorkingMemoryDuration)) {
                            swal("Invalid!", "Duration memory or transition value cannot be decimal", "warning", { closeOnClickOutside: false });
                        } else {
                            confirmMessageAdd();
                            $('.swal-button--defeat').on('click', function () {
                                LoadingMask.show();
            ...
            if (Valid == false) {
                ...
                if (SubTestCode == '' || SubTestCode == undefined ||
                    ...
                    if (WorkingMemoryDuration <= 0) {
                        ...
                        const IsDecimal = (num) => num.toString().includes(".");
                        if (IsDecimal(WorkingMemoryDuration) || IsDecimal(WorkingMemoryDurationTransition)) {
                            swal("Invalid!", "Duration memory or transition value cannot be decimal", "warning", { closeOnClickOutside: false });
                        } else {
                            confirmMessageAdd();
                            $('.swal-button--defeat').on('click', function () {
        ...
        //var checkinputAllScoreZero = ifAnyNonZero(VerbalLists, lengthVerbal);
        var Valid;
        var checkinputAllScoreZero = ifAnyNonZero(VerbalLists, lengthVerbal);
        ...
        //        } 
        //    } else {
        //        if (VerbalLists[kll].Text != "" && (VerbalLists[kll].AnswerScore !== "" && VerbalLists[kll].AnswerScore >= 0)) {
        //            Valid = true;
        //        }
        //        else {
        //            Valid = false;
        ...
        if (QuestionCode == '') {
            if (Valid == false) {
                ...
                if (HeaderCheck || VerbalQuestion == '' || VerbalLayout == "" || VerbalLayout == undefined) {
                    swal("Incompleted Data", "Please Fill Mandatory Field", "warning", { closeOnClickOutside: false });
                }else if(viewModel.Story.StoryList == "" || viewModel.Story.StoryList == 0){
                    swal("Incompleted Data", "Please Select Story Question Field", "warning", { closeOnClickOutside: false });
                }
                else {
                    confirmMessageAdd();
                    $('.swal-button--defeat').on('click', function () {
...
function checkValidationForm(dtArr, length){
    var valid = true;
    for (kll = 0; kll < length; kll++) {
        if (dtArr[kll].Text != "" && (dtArr[kll].AnswerScore !== "" && parseInt(dtArr[kll].AnswerScore) >= 0)) {
            valid = true;
        }
        else {
            valid = false;
...
function ifAnyNonZeroImage(array, length){
    var dt = [];
    for(var i = 0; i< length; ++i){
        if(array.NormalImage[i].NormalScoreImage == "0" || array.NormalImage[i].NormalScoreImage === 0){
            dt.push(true);
        }else{
            dt.push(array.NormalImage[i].NormalScoreImage + i);
        }
    }
    return !!dt.reduce(function(a, b){return (a === b) ? a : NaN; });
}

function ifAnyNonZero(array, length){
    var dt = [];
    for(var i = 0; i< length; ++i){
        if(array[i].AnswerScore == "0" || array[i].AnswerScore === 0){
            dt.push(true);
        }else{
            dt.push(array[i].AnswerScore + i);
        }
    }
    return !!dt.reduce(function(a, b){return (a === b) ? a : NaN; });
}