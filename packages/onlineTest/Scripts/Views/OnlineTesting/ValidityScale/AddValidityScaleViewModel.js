...
save = function () {
    ...
    if (document.getElementById('IsByMappingQuestion').checked) {
        viewModel.set("IsByMappingQuestion", true);
        viewModel.set("IsBySubTest", false);
        viewModel.set("TestToolName", TestToolName);
    }

    if (ValidityScaleName == '' || TestTypeName == '' || TestTool == '' || (viewModel.IsBySubTest == false && viewModel.IsByMappingQuestion == false)) {
        LoadingMask.hide();
        ...
        if (viewModel.DetailBySubTest.SubTestCode == "" || viewModel.DetailBySubTest.SubTestName == "" || SubTestName == "") {
            ...
            $('.swal-button--defeat').on('click', function () {
                ...
                $.ajax({
                    type: "POST",
                    url: SERVICE_URL + "api/ValidityScale/Save",
                    headers: { "Authorization-Token": Cookie.load() },
                    data: {
